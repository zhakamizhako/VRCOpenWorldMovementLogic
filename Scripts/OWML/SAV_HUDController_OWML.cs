...
{
    [Header("Insert the old HUD Controller here.")]
    [Header("Leave it empty if you want this to work standalone.")]
    [InspectorName("Old HUD Controller")]
    public SAV_HUDController HBOld;
    
    [Header("Insert the aircraft's OWML Script here.")]
    public ZHK_OpenWorldMovementLogic OWML;
    
    [Header("Do not fill up these unless if you plan to replace the HUD Controller.")]
    ...
    private void Start()
    {
        ...
        if (HBOld != null) // Adapt old hud controller
        {
            PilotSeatAdjusterTarget = HBOld.PilotSeatAdjusterTarget;
            SAVControl = HBOld.SAVControl;
            HUDAnimator = HBOld.HUDAnimator;
            HUDText_G = HBOld.HUDText_G;
            HUDText_mach = HBOld.HUDText_mach;
            HUDText_altitude = HBOld.HUDText_altitude;
            HUDText_radaraltitude = HBOld.HUDText_radaraltitude;
            RadarAltitudeOffset = HBOld.RadarAltitudeOffset;
            AltitudeConversion = HBOld.AltitudeConversion;
            HUDText_knots = HBOld.HUDText_knots;
            SpeedConversion = HBOld.SpeedConversion;
            HUDText_knotsairspeed = HBOld.HUDText_knotsairspeed;
            HUDText_angleofattack = HBOld.HUDText_angleofattack;
            DownIndicator = HBOld.DownIndicator;
            ElevationIndicator = HBOld.ElevationIndicator;
            HeadingIndicator = HBOld.HeadingIndicator;
            VelocityIndicator = HBOld.VelocityIndicator;
            distance_from_head = HBOld.distance_from_head;
            HBOld.gameObject.SetActive(true);
            HBOld.gameObject.GetComponent<UdonBehaviour>().enabled = false;
        }
