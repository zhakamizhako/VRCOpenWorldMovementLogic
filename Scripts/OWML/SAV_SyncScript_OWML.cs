...
/*
Modified SyncScript for OWML
For SaccFlight 1.5

Based on the original SAV_SyncScript
...
*/
public class SAV_SyncScript_OWML : UdonSharpBehaviour
{
    // whispers to Zwei, "it's okay"
    public UdonSharpBehaviour SAVControl;
	[HideInInspector]public SaccAirVehicle SAVProper;
    ...
    [HideInInspector]public Vector3 L_PingAdjustedPosition = Vector3.zero;
    [HideInInspector]public Vector3 L_LastPingAdjustedPosition;
    ...
    private void Start()
    {
        ...
        SAVProper = (SaccAirVehicle)SAVControl;
        ...
        if (IsOwner)//send data
        {
            if (StartStopWatch.Elapsed.TotalSeconds > nextUpdateTime - (Time.deltaTime * .5f))
            {
                if (!Networking.IsClogged || Piloting)
                {
                    ...
                    Still = !Piloting && ((((OWML ? OWML.Map.position + VehicleTransform.position : VehicleTransform.position) - O_Position).magnitude < IdleMovementRange) && Quaternion.Angle(VehicleTransform.rotation, O_Rotation_Q) < IdleRotationRange);
                    ...
                    O_Position = OWML ? (OWML.PosSync) : VehicleTransform.position;
    ...
    public override void OnDeserialization()
    {
        if (O_UpdateTime != O_LastUpdateTime && !IsOwner)//only do anything if OnDeserialization was for this script
        {
            ...
            L_PingAdjustedPosition = (OWML ?
                                    (O_Position + OWML.Map.position )
                                    : O_Position) + ((CurrentVelocity + Acceleration) * Ping);
