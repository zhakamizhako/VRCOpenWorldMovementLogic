...
public class SAV_SyncScript_OWML : UdonSharpBehaviour
{
    ...
    public Vector3 L_PingAdjustedPosition = Vector3.zero;
    ...
    public Vector3 Extrapolation_Raw;
        ...
        if (IsOwner) //send data
        {
            ...
            if (time > nextUpdateTime - (Time.deltaTime * .5f))
            {
                if (!Networking.IsClogged || Piloting)
                {
                    ...
                    bool Still = !Piloting && Grounded &&
                                 ((((OWML ? OWML.Map.position + VehicleTransform.position : VehicleTransform.position) -
                                    O_Position).magnitude < IdleMovementRange) &&
                                  Quaternion.Angle(VehicleTransform.rotation, O_Rotation_Q) < IdleRotationRange);
